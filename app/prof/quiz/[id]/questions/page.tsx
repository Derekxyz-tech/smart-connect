      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-200 pl-14 pr-4 md:px-8 py-6 shadow-sm z-10">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-semibold text-slate-800 tracking-tight truncate">{quiz.titre}</h1>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex-shrink-0 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors"
              >
                <span className="hidden sm:inline">Annuler</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="sm:hidden"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
              </button>
              <button
                onClick={handleSaveQuiz}
                disabled={saving || !quiz.questions || quiz.questions.length === 0}
                className="flex-shrink-0 px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {saving ? '...' : <><span className="hidden sm:inline">Sauvegarder</span><span className="sm:hidden">OK</span></>}
              </button>
            </div>
          </div>
        </header>
