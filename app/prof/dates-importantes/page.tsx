...
export default async function ProfDatesImportantesPage() {
    ...
    <div className="h-screen flex overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-center pl-14 pr-4 md:px-8 shadow-sm z-10">
          <h2 className="text-xl font-semibold text-slate-800 tracking-tight truncate">Dates Importantes</h2>
        </header>
            ...
            {evenements && evenements.length > 0 ? (
              <div className="space-y-4">
                {evenements.map((event: any) => {
                  ...
                  const typeBadge = getTypeBadge(event.type || 'evenement')
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h3 className="text-lg font-semibold text-slate-800 mb-1">{event.titre}</h3>
                          ...
                          {event.description && (
                            <p className="text-slate-700 whitespace-pre-wrap break-words">{event.description}</p>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
                ...
                <p className="text-sm text-slate-400 mt-2">Les dates importantes apparaîtront ici lorsqu&apos;elles seront ajoutées par l&apos;administration.</p>
              </div>
            )}
          </div>
        </main>
