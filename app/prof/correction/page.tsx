...
function CorrectionContent() {
  ...
  useEffect(() => {
    ...
    setUser(contextUser)
    setLoading(false)
    loadSoumissions(contextUser.id)
    loadOptions(contextUser.id)
    ...
    <div className="h-screen flex overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-200 h-16 flex items-center relative pl-14 pr-4 md:px-8 shadow-sm z-10">
          <h2 className="text-xl font-semibold text-slate-800 tracking-tight truncate flex-1 text-center">Correction & Notes</h2>
          <button className="flex-shrink-0 flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">
